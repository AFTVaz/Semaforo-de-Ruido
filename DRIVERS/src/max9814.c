...
#define ADC_RESOLUTION   		(ADC_MAX_VOLTAGE / ADC_MAX_DIVISIONS)
...
static void MAX9814_Calculate_Sensitivity( void ) {
    float mic_sensitivity_v = powf(10.0f, MAX9814_SENSITIVITY / 20.0f);
    mic_ref_Vout = mic_sensitivity_v * powf(10.0f, mic_gain_db / 20.0f);
}
...
static float MAX9814_Conversion_To_DB( float adc_avg_value ){
	...
	return 20.0f * log10f(v_out / mic_ref_Vout) + MAX9814_DB_SPL_REF;
}
...
float MAX9814_Read(void) {
	...
	float adc_avg = MAX9814_Calculate_ADC_Average(mic_buffer, MAX9814_NUM_SAMPLES, adc_offset);
	return MAX9814_Conversion_To_DB(adc_avg);
}
